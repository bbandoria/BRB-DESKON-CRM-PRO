...
// Destino B (mantenedor, Vercel — ver docs/DEPLOY-CHECKLIST.md §B): não tem
...
/** As rotas que o `vercel.json` agenda via `crons[].path`. */
function rotasAgendadasNaVercel(): string[] {
  const json = readFileSync(VERCEL_JSON, "utf8");
  const achadas = json.matchAll(/"path":\s*"\/api\/v1\/cron\/([a-z0-9-]+)/g);
  return [...new Set([...achadas].map((m) => m[1]!))].sort();
}
...
describe("rotas de cron × agendamento no self-host", () => {
  ...
  it("toda rota de cron do código está agendada no vercel.json (destino B)", () => {
    ...
    expect(
      naoAgendadas,
      `Rota(s) de cron sem entrada em crons[] no vercel.json: ${naoAgendadas.join(", ")}. ` +
        `No deploy Vercel (docs/DEPLOY-CHECKLIST.md §B) elas nunca rodam sozinhas.`,
    ).toEqual([]);
  });

  it("todo agendamento do vercel.json aponta para uma rota que existe", () => {
    const orfas = rotasAgendadasNaVercel().filter((r) => !rotasNoCodigo().includes(r));
    expect(
      orfas,
      `vercel.json agenda rota(s) que não existem mais: ${orfas.join(", ")}.`,
    ).toEqual([]);
  });
});
